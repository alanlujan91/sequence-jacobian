...
def apply_binary_op(op, a1, a2):
    if getattr(a1, op)(a2) is not NotImplemented:
        ...
        raise NotImplementedError(
            f"{op} cannot be performed between {a1} and {a2} directly, and no"
...
def apply_op(op, *args):
    if len(args) == 1:
        ...
        raise ValueError(
            f"apply_op only supports unary or binary operators currently. {len(args)} is an invalid"
...
def test_ignore():
    ...
    for t1 in arg_singles:
        for op in ["__neg__", "__pos__"]:
            ...
            assert np.all(
                numeric_primitive(apply_op(op, t1))
                == apply_op(op, numeric_primitive(t1))
            )
    ...
    arg_pairs = [
        (IgnoreInt(1), 1),
        (1, IgnoreInt(1)),
        (IgnoreInt(1), IgnoreInt(2)),
        (IgnoreInt(1)(-1), 1),
        (1, IgnoreInt(1)(-1)),
        (IgnoreInt(1)(-1), IgnoreInt(2)),
        (IgnoreInt(1), IgnoreInt(2)(-1)),
    ]
    for pair in arg_pairs:
        ...
        for op in ["__add__", "__radd__", "__sub__", "__rsub__", "__mul__", "__rmul__"]:
            ...
            if op in ["__radd__", "__rsub__", "__rmul__"] and not (
                type(t1) == int and type(t2) == IgnoreInt
            ):
                assert type(apply_op(op, t1, t2)) == IgnoreInt
            assert np.all(
                numeric_primitive(apply_op(op, t1, t2))
                == apply_op(op, numeric_primitive(t1), numeric_primitive(t2))
            )

    arg_pairs = [
        (IgnoreFloat(1), 1),
        (1, IgnoreFloat(1)),
        (IgnoreFloat(1), IgnoreFloat(2)),
        (IgnoreFloat(1)(-1), 1),
        (1, IgnoreFloat(1)(-1)),
        (IgnoreFloat(1)(-1), IgnoreFloat(2)),
        (IgnoreFloat(1), IgnoreFloat(2)(-1)),
    ]
    for pair in arg_pairs:
        t1, t2 = pair
        for op in [
            ...
            if op in ["__radd__", "__rsub__", "__rmul__", "__rpow__"] and not (
                type(t1) == float and type(t2) == IgnoreFloat
            ):
...
def test_ignore_vector():
    # Test unary operations
    arg_singles = [
        IgnoreVector(np.array([1, 2, 3])),
        IgnoreVector(np.array([1, 2, 3]))(-1),
    ]
    for t1 in arg_singles:
        for op in ["__neg__", "__pos__"]:
...
def test_displace():
    ...
    # Test unary operations
    arg_singles = [
        Displace(np.array([1, 2, 3]), 2, 2),
        Displace(np.array([1, 2, 3]), 2, 2)(-1),
    ]
    for t1 in arg_singles:
        for op in ["__neg__", "__pos__"]:
    ...
    for pair in arg_pairs:
        ...
        for op in [
            ...
            assert np.all(
                numeric_primitive(apply_op(op, t1, t2).ss)
                == apply_op(
                    op,
                    t1.ss if isinstance(t1, Displace) else numeric_primitive(t1),
...
def test_accumulated_derivative():
    ...
    arg_singles = [
        AccumulatedDerivative(),
        AccumulatedDerivative()(-1),
        AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0),
    ]
    for t1 in arg_singles:
        for op in ["__neg__", "__pos__"]:
            assert type(apply_op(op, t1)) == AccumulatedDerivative
            assert np.all(
                np.fromiter(apply_op(op, t1).elements.values(), dtype=float)
    ...
    # Test binary operations
    arg_pairs = [
        (AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0), 3),
        (AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0), IgnoreFloat(3)),
        (3, AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0)),
        (IgnoreFloat(3), AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0)),
        (
            AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0),
            AccumulatedDerivative(elements={(1, 1): 4.0}, f_value=5.0),
        ),
        ...
        (AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0)(-1), 3),
        (
            AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0)(-1),
            IgnoreFloat(3),
        ),
        (3, AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0)(-1)),
        (
            IgnoreFloat(3),
            AccumulatedDerivative(elements={(1, 1): 2.0}, f_value=2.0)(-1),
        ),
    ...
    for pair in arg_pairs:
        ...
        for op in [
            ...
            if isinstance(t1, AccumulatedDerivative) and isinstance(
                t2, AccumulatedDerivative
            ):
                assert apply_op(op, t1, t2).f_value == apply_op(
                    op, t1.f_value, t2.f_value
                )
                ...
                if op in ["__add__", "__radd__", "__sub__", "__rsub__"]:
                    ...
                    assert result == get_fp_value(
                        t1
                    ) * t2.f_value + t1.f_value * get_fp_value(t2)
                elif op == "__truediv__":
                    assert (
                        result
                        == (
                            t2.f_value * get_fp_value(t1)
                            - t1.f_value * get_fp_value(t2)
                        )
                        / t2.f_value**2
                    )
                    ...
                    assert result == (t1.f_value ** (t2.f_value - 1)) * (
                        t2.f_value * get_fp_value(t1)
                        + t1.f_value * np.log(t1.f_value) * get_fp_value(t2)
                    )
                else:  # op == "__rpow__":
                    assert result == (t2.f_value ** (t1.f_value - 1)) * (
                        t1.f_value * get_fp_value(t2)
                        + t2.f_value * np.log(t2.f_value) * get_fp_value(t1)
                    )
            else:
                assert (
                    apply_op(op, t1, t2).f_value
                    == apply_op(op, t1.f_value, numeric_primitive(t2))
                    if isinstance(t1, AccumulatedDerivative)
                    else apply_op(op, numeric_primitive(t1), t2.f_value)
                )

                if op in ["__add__", "__radd__", "__sub__"]:
                    assert (
                        result == get_fp_value(t1)
                        if isinstance(t1, AccumulatedDerivative)
                        else get_fp_value(t2)
                    )
                elif op == "__rsub__":
                    assert (
                        result == -get_fp_value(t1)
                    ...
                    assert (
                        result == numeric_primitive(t2) * get_fp_value(t1)
                        if isinstance(t1, AccumulatedDerivative)
                        else numeric_primitive(t1) * get_fp_value(t2)
                    )
                elif op == "__truediv__":
                    assert (
                        result == get_fp_value(t1) / numeric_primitive(t2)
                        if isinstance(t1, AccumulatedDerivative)
                    ...
                    assert (
                        result
                        == -numeric_primitive(t2) / t1.f_value**2 * get_fp_value(t1)
                        if isinstance(t1, AccumulatedDerivative)
                        else get_fp_value(t2) / numeric_primitive(t1)
                    )
                elif op == "__pow__":
                    assert (
                        result
                        == numeric_primitive(t2)
                        * t1.f_value ** (numeric_primitive(t2) - 1)
                        * get_fp_value(t1)
                        if isinstance(t1, AccumulatedDerivative)
                        else np.log(numeric_primitive(t1))
                        * numeric_primitive(t1) ** t2.f_value
                        * get_fp_value(t2)
                    )
                else:  # op == "__rpow__"
                    assert (
                        result
                        == np.log(numeric_primitive(t2))
                        * numeric_primitive(t2) ** t1.f_value
                        * get_fp_value(t1)
                        if isinstance(t1, AccumulatedDerivative)
                        else numeric_primitive(t1)
                        * t2.f_value ** (numeric_primitive(t1) - 1)
                        * get_fp_value(t2)
                    )
