...
@pytest.mark.parametrize(
    "block,ss",
    [
        (F, SteadyStateDict({"K": 1, "L": 1, "Z": 1, "alpha": 0.5})),
        (
            investment,
            SteadyStateDict(
                {
...
def test_block_consistency(block, ss):
    ...
    td_results = block.impulse_nonlinear(
        ss_results, {k: np.zeros(20) for k in ss.keys()}
    )
    ...
    td_up = block.impulse_nonlinear(
        ss_results, {i: h * shock for i, shock in all_shocks.items()}
    )
    td_dn = block.impulse_nonlinear(
        ss_results, {i: -h * shock for i, shock in all_shocks.items()}
    )

    linear_impulses = {o: (td_up[o] - td_dn[o]) / (2 * h) for o in block.outputs}
    linear_impulses_from_jac = {
        o: sum(J[o][i] @ all_shocks[i] for i in all_shocks if i in J[o])
