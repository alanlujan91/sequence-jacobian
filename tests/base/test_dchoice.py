...
from sequence_jacobian.utilities.interpolate import (
    interpolate_coord_njit,
    apply_coord_njit,
    interpolate_point,
)
...
def consav(V, Va, a_grid, coh, y, r, beta, eis):
    ...
    uc_endo = beta * Va
    c_endo = uc_endo ** (-eis)
    a_endo = (c_endo + a_grid[np.newaxis, np.newaxis, :] - y[:, :, np.newaxis]) / (
        1 + r
    )
...
def upper_envelope_core(ilower, iupper, W, a_endo, c_endo, coh, a_grid, *args):
    ...
    for ib in range(nB):
        ...
        if ilower_cur > 0:
            ai, api = interpolate_coord_njit(
                a_endo[ib, :ilower_cur], a_grid[:ilower_cur]
            )
        ...
        for ia in range(ilower_cur, iupper_cur):
            ...
            for ja in range(nA - 1):
                ...
                if interp or extrap:
                    c0 = interpolate_point(
                        acur, ap_low, ap_high, c_endo[ib, ja], c_endo[ib, ja + 1]
                    )
                    W0 = interpolate_point(
                        acur, ap_low, ap_high, W[ib, ja], W[ib, ja + 1]
                    )
                    V0 = util(c0, *args) + W0
        ...
        if iupper_cur > 0:
            ai, api = interpolate_coord_njit(
                a_endo[ib, iupper_cur:], a_grid[iupper_cur:]
            )
...
hh = StageBlock(
    [prod_stage, search_stage, labsup_stage, consav_stage],
    backward_init=backward_init,
    hetinputs=[make_grids, labor_income],
    name="household",
)
...
def test_runs():
    ...
    assert np.allclose(
        ss1.internals["household"]["consav"]["D"],
        ss2.internals["household"]["consav"]["D"],
    )
    assert np.allclose(
        ss1.internals["household"]["consav"]["a"],
        ss2.internals["household"]["consav"]["a"],
    )
    assert np.allclose(
        ss1.internals["household"]["consav"]["c"],
        ss2.internals["household"]["consav"]["c"],
    )
