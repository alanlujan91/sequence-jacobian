...
def union(UCE, tau, w, N, pi, muw, kappaw, nu, vphi, beta):
    wnkpc = (
        kappaw * N * (vphi * N**nu - (1 - tau) * w * UCE / muw)
        + beta * (1 + pi(+1)).apply(np.log)
        - (1 + pi).apply(np.log)
    )
    return wnkpc
...
def union_ss(atw, UCE, muw, N, nu, kappaw, beta, pi):
    ...
    wnkpc = (
        kappaw * N * (vphi * N**nu - atw * UCE / muw)
        + beta * (1 + pi(+1)).apply(np.log)
        - (1 + pi).apply(np.log)
    )
    return wnkpc, vphi
...
def test_all():
    ...
    household_ha = household_ha.add_hetoutputs([mpcs, weighted_uc]).rename(
        "household_ha"
    )
    ...
    ss["ha"] = dag_ss["ha"].solve_steady_state(
        calibration,
        dissolve=["fiscal"],
        solver="hybr",
    ...
    Js = {"ra": {}, "ha": {}}
    Js["ha"]["household_ha"] = household_ha.jacobian(
        ss["ha"], inputs=["N", "atw", "r", "transfer"], outputs=["C", "A", "UCE"], T=300
    )
    ...
    for k in ["ra", "ha"]:
        G[k] = dag[k].solve_jacobian(
            ss[k], unknowns, targets, inputs=["G"], T=300, Js=Js[k]
        )
        td_lin1[k] = G[k] @ shock
        td_lin2[k] = dag[k].solve_impulse_linear(
            ss[k], unknowns, targets, shock, Js=Js[k]
        )
    ...
    td_nonlin = dag["ha"].solve_impulse_nonlinear(
        ss["ha"],
        unknowns,
        targets,
        inputs=shock * 1e-2,
        Js=Js,
        internals=["household_ha"],
        ss_initial=ss["ha"],
    )
    ...
    td_A = np.sum(
        td_nonlin_lvl.internals["household_ha"]["a"]
        * td_nonlin_lvl.internals["household_ha"]["D"],
        axis=(1, 2),
    )
