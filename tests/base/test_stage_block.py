...
hh2 = StageBlock(
    [ExogenousMaker("Pi", 0, "stage0"), het_stage],
    name="hh",
...
def test_equivalence():
    hh1 = hh.add_hetinputs([make_grids, income, alter_Pi]).add_hetoutputs(
        [marginal_utility]
    )
    calibration = {
        "r": 0.004,
...
def test_remap():
    ...
    hh1_women = hh1.remap({k: k + "_women" for k in hh1.outputs | ["sd_e"]}).rename(
        "women"
    )
    ...
    hh2_women = hh2.remap({k: k + "_women" for k in hh2.outputs | ["sd_e"]}).rename(
        "women"
    )
