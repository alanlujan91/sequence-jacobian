...
def test_rbc_td(rbc_dag):
    ...
    td_nonlin = rbc_model.solve_impulse_nonlinear(
        ss, unknowns, targets, inputs={"Z": dZ[:, 0]}, outputs=["C"]
    )
    td_nonlin_news = rbc_model.solve_impulse_nonlinear(
        ss, unknowns, targets, inputs={"Z": dZ[:, 1]}, outputs=["C"]
    )
...
def test_hank_td(one_asset_hank_dag):
    ...
    G = hank_model.solve_jacobian(
        ss, unknowns, targets, exogenous, T=T, Js={"hh": J_ha}
    )

    rho_r, sig_r = 0.61, -0.01 / 4
    drstar = sig_r * rho_r ** (np.arange(T))

    td_nonlin = hank_model.solve_impulse_nonlinear(
        ss, unknowns, targets, {"rstar": drstar}, Js={"hh": J_ha}
    )
...
def test_two_asset_td(two_asset_hank_dag):
    ...
    G = two_asset_model.solve_jacobian(
        ss, unknowns, targets, exogenous, T=T, Js={"hh": J_ha}
    )
    ...
    for shock_size, tol in [(0.1, 3e-4), (1, 2e-2)]:
        ...
        td_nonlin = two_asset_model.solve_impulse_nonlinear(
            ss, unknowns, targets, {"rstar": drstar}, Js={"hh": J_ha}
        )
...
def test_two_asset_solved_v_simple_td(two_asset_hank_dag):
    ...
    blocks_simple = [
        ...
        two_asset.arbitrage,
        two_asset.labor,
        two_asset.investment,
        two_asset.dividend,
        two_asset.taylor,
        two_asset.fiscal,
        two_asset.share_value,
        two_asset.finance,
        two_asset.wage,
        two_asset.union,
        two_asset.mkt_clearing,
    ]
    two_asset_model_simple = combine(
        blocks_simple, name="Two-Asset HANK w/ SimpleBlocks"
    )
    ...
    G_simple = two_asset_model_simple.solve_jacobian(
        ss, unknowns_simple, targets_simple, exogenous, T=T, Js={"hh": J_ha}
    )
    ...
    td_nonlin_simple = two_asset_model_simple.solve_impulse_nonlinear(
        ss, unknowns_simple, targets_simple, {"rstar": drstar}, Js={"hh": J_ha}
    )
