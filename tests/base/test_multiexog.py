...
@het(
    exogenous=["Pi_e", "Pi_z"], policy="a", backward="Va", backward_init=household_init
)
def household_multidim(Va_p, a_grid, y, r, beta, sigma):
    c_nextgrid = (beta * Va_p) ** (-1 / sigma)
    coh = (1 + r) * a_grid + y[..., np.newaxis]
    a = sj.utilities.interpolate.interpolate_y(
        c_nextgrid + a_grid, coh, a_grid
    )  # (x, xq, y)
...
def test_equivalence():
    calibration = dict(
        beta=0.95, r=0.01, sigma=2, a_grid=sj.utilities.discretize.agrid(1000, 50)
    )
    ...
    ss_multidim = household_multidim.steady_state(
        {**calibration, "y": e_multidim, "Pi_e": Pi1, "Pi_z": Pi2}
    )
    ss_onedim = household_onedim.steady_state({**calibration, "y": e_onedim, "Pi": Pi})

    assert np.isclose(ss_multidim["A"], ss_onedim["A"]) and np.isclose(
        ss_multidim["C"], ss_onedim["C"]
    )
    ...
    J_multidim = household_multidim.jacobian(
        ss_multidim, inputs=["r"], outputs=["A"], T=10
    )
