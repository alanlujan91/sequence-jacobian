...
from .support.steady_state import (
    provide_solver_default,
    solve_for_unknowns,
...
class Block:
    ...
    def steady_state(
        self,
        calibration: Union[SteadyStateDict, UserProvidedSS],
        dissolve: List[str] = [],
        ...
        **kwargs,
        ...
        if isinstance(self, Parent):
            return self.M @ self._steady_state(
                self.M.inv @ calibration,
                dissolve=dissolve,
                options=options,
                **own_options,
            )
    ...
    def impulse_nonlinear(
        self,
        ss: SteadyStateDict,
        inputs: Union[Dict[str, Array], ImpulseDict],
        outputs: Optional[List[str]] = None,
        internals: Union[Dict[str, List[str]], List[str]] = {},
        Js: Dict[str, JacobianDict] = {},
        options: Dict[str, dict] = {},
        ss_initial: Optional[SteadyStateDict] = None,
        **kwargs,
    ) -> ImpulseDict:
        ...
        actual_outputs, inputs_as_outputs = self.process_outputs(
            ss, self.make_ordered_set(inputs), self.make_ordered_set(outputs)
        )

        if isinstance(self, Parent):
            # SolvedBlocks may use Js and may be nested in a CombinedBlock, so we need to pass them down to any parent
            out = self.M @ self._impulse_nonlinear(
                self.M.inv @ ss,
                self.M.inv @ inputs,
                self.M.inv @ actual_outputs,
                internals,
                Js,
                options,
                self.M.inv @ ss_initial,
                **own_options,
            )
            ...
            out = self.M @ self._impulse_nonlinear(
                ...
                self.M.inv @ inputs,
                self.M.inv @ actual_outputs,
                self.internals_to_report(internals),
                self.M.inv @ ss_initial,
                **own_options,
            )
        else:
    ...
    def impulse_linear(
        ...
        **kwargs,
        ...
        if isinstance(self, Parent):
            out = self.M @ self._impulse_linear(
                self.M.inv @ ss,
                self.M.inv @ inputs,
                self.M.inv @ actual_outputs,
                Js,
    ...
    def partial_jacobians(
        ...
        **kwargs,
        ...
        if (
            (self.name in Js)
            and isinstance(Js[self.name], JacobianDict)
            and (inputs <= Js[self.name].inputs)
            and (outputs <= Js[self.name].outputs)
        ):
        ...
        partial = self._partial_jacobians(
            self.M.inv @ ss,
            self.M.inv @ inputs,
            self.M.inv @ outputs,
            T,
    ...
    solve_steady_state_options = dict(
        solver="",
        solver_kwargs={},
        ttol=1e-12,
        ctol=1e-9,
    ...
    def solve_steady_state(
        self,
        calibration: Dict[str, Union[Real, Array]],
        ...
        **kwargs,
        ...
        solver = (
            options["solver"] if options["solver"] else provide_solver_default(unknowns)
        )
        ...
        def residual(unknown_values, unknowns_keys=unknowns.keys(), targets=targets):
            ...
            ss.update(
                self.steady_state(ss, dissolve=dissolve, options=options, **kwargs)
            )
            return compute_target_values(targets, ss)

        _ = solve_for_unknowns(
            residual,
            unknowns,
            solver,
    ...
    def solve_impulse_nonlinear(
        ...
        Js: Dict[str, JacobianDict] = {},
        options: Dict[str, dict] = {},
        H_U_factored: Optional[FactoredJacobianDict] = None,
        ss_initial: Optional[SteadyStateDict] = None,
        **kwargs,
    ) -> ImpulseDict:
        ...
        actual_outputs, inputs_as_outputs = self.process_outputs(
            ss, input_names | unknowns, self.make_ordered_set(outputs)
        )

        T = inputs.T

        Js = self.partial_jacobians(
            ss,
            input_names | unknowns,
        ...
        for it in range(options["maxit"]):
            results = self.impulse_nonlinear(
                ss,
                inputs | U,
                actual_outputs | targets,
                internals,
                Js,
                options,
                ss_initial,
            ...
            raise ValueError(
                f'No convergence after {options["maxit"]} backward iterations!'
            )
    ...
    def solve_impulse_linear(
        ...
        **kwargs,
        ...
        dH = self.impulse_linear(ss, inputs, targets, Js, options, **kwargs).get(
            targets
        )  # .get(targets) fills in zeros
        ...
        return (inputs | dU)[inputs_as_outputs] | self.impulse_linear(
            ss, dU | inputs, actual_outputs, Js, options, **kwargs
        )
    ...
    def solve_jacobian(
        ...
        **kwargs,
        ...
        variables to be solved for and the target conditions that must hold in general equilibrium
        """
        inputs, unknowns = self.make_ordered_set(inputs), self.make_ordered_set(
            unknowns
        )
        actual_outputs, unknowns_as_outputs = self.process_outputs(
            ss, unknowns, self.make_ordered_set(outputs)
        )
        ...
        if H_U_factored is None:
            ...
            U_Z = JacobianDict.unpack(
                -np.linalg.solve(H_U, H_Z.pack(T)), unknowns, inputs, T
            )
        else:
        ...
        return self_with_unknowns.jacobian(
            ss, inputs, unknowns_as_outputs | actual_outputs, T, Js, options, **kwargs
        )
