...
class JacobianDictBlock(JacobianDict, Block):
    ...
    def _jacobian(self, ss, inputs, outputs, T):
        if not inputs <= self.inputs:
            raise KeyError(
                f"Asking JacobianDictBlock for {inputs - self.inputs}, which are among its inputs {self.inputs}"
            )
        if not outputs <= self.outputs:
            raise KeyError(
                f"Asking JacobianDictBlock for {outputs - self.outputs}, which are among its outputs {self.outputs}"
            )
