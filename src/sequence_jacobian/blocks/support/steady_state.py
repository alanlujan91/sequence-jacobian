...
def instantiate_steady_state_mutable_kwargs(
...
def provide_solver_default(unknowns):
    if len(unknowns) == 1:
        ...
        if not isinstance(bounds, tuple) or bounds[0] > bounds[1]:
            raise ValueError(
                "Unable to find a compatible one-dimensional solver with provided `unknowns`.\n"
        ...
        if not np.all([isinstance(v, Real) for v in init_values]):
            raise ValueError(
                "Unable to find a compatible multi-dimensional solver with provided `unknowns`.\n"
        ...
        raise ValueError(
            "`unknowns` is empty! Please provide a dict of keys/values equal to the number of unknowns"
...
def run_consistency_check(cresid, ctol=1e-9, fragile=False):
    if cresid > ctol:
        if fragile:
            raise RuntimeError(
                f"The target values evaluated for the proposed set of unknowns produce a "
...
def compare_steady_states(
    ...
    if internal:
        if not hasattr(ss_ref, "internal") or not hasattr(ss_comp, "internal"):
            warnings.warn(
                "The provided steady state dicts do not both have .internal attrs. Will only compare"
                " top-level values"
            )
            ds_to_check = [(ss_ref, ss_comp, "toplevel")]
        else:
            ds_to_check = [(ss_ref, ss_comp, "toplevel")] + [
                (ss_ref.internal[i], ss_comp.internal[i], i + "_internal")
                for i in ss_ref.internal
            ]
    else:
        ds_to_check = [(ss_ref, ss_comp, "toplevel")]
    ...
    for ds in ds_to_check:
        ...
        for key_ref in d_ref.keys():
            ...
            if np.isscalar(d_ref[key_ref]):
                ...
                resid = np.linalg.norm(
                    d_ref[key_ref].ravel() - d_comp[key_comp].ravel(), np.inf
                )
            if verbose:
                print(f"{key_ref} resid: {resid}")
            else:
                if not np.all(np.isclose(resid, 0.0, atol=tol)):
                    valid = False
        ...
        if check_same_keys:
            ...
            if diff_keys:
                if verbose:
                    print(
                        f"At level '{level}', the keys present only one of the two steady state dicts are {diff_keys}"
                    )
                valid = False
...
def solve_for_unknowns(
    ...
    if solver is None:
        raise RuntimeError(
            "Must provide a numerical solver from the following set: brentq, broyden, solved"
        )
        ...
        initial_values, bounds = extract_multivariate_initial_values_and_bounds(
            unknowns
        )
        # If no bounds were provided
        if not bounds:
            result = opt.root(
                residual_f, initial_values, method=solver, tol=tol, **solver_kwargs
            )
        else:
            constrained_residual = constrained_multivariate_residual(
                residual_f,
                bounds,
                verbose=verbose,
        ...
        if not result.success:
            raise ValueError(
                f"Steady-state solver, {solver}, did not converge."
    ...
    #  everytime a new custom solver is implemented.
        ...
        # If no bounds were provided
        if not bounds:
            unknown_solutions, _ = solvers.broyden_solver(
                residual_f, initial_values, tol=tol, verbose=verbose, **solver_kwargs
            )
        else:
            constrained_residual = constrained_multivariate_residual(
                residual_f,
        ...
        # If no bounds were provided
        if not bounds:
            unknown_solutions, _ = solvers.newton_solver(
                residual_f, initial_values, tol=tol, verbose=verbose, **solver_kwargs
            )
        else:
            constrained_residual = constrained_multivariate_residual(
...
def extract_multivariate_initial_values_and_bounds(unknowns, fragile=False):
    ...
    for k, v in unknowns.items():
        if np.isscalar(v):
            ...
            if fragile:
                raise ValueError(
                    f"{len(v)} is an invalid size for the value of an unknown."
                ...
                warnings.warn(
                    "Interpreting values of `unknowns` from length 2 tuple as lower and upper bounds"
...
def residual_with_linear_continuation(
    residual,
    bounds,
    eval_at_boundary=False,
    boundary_epsilon=1e-4,
    penalty_scale=1e1,
    verbose=False,
):
    ...
    def constr_residual(x, residual_cache=[]):
        ...
        residual_with_boundary_penalty = (
            residual_censored + (x - x_censored) * penalty_scale * residual_censored
        )
...
def constrained_multivariate_residual(
    residual, bounds, method="linear_continuation", verbose=False, **constrained_kwargs
):
    ...
    if method == "linear_continuation":
        return residual_with_linear_continuation(
            residual, bounds, verbose=verbose, **constrained_kwargs
        )
    # TODO: Implement logistic transform as another option for constrained multivariate residual
    else:
        raise ValueError(
            f"Method {method} for constrained multivariate root-finding has not yet been implemented."
        )
