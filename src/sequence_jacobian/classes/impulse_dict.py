...
class ImpulseDict(ResultDict):
    def __init__(self, data, internals=None, T=None):
        if isinstance(data, ImpulseDict):
            if internals is not None or T is not None:
                raise ValueError(
                    "Supplying ImpulseDict and also internal or T to constructor not allowed"
                )
            super().__init__(data)
            self.T = data.T
        else:
            if not isinstance(data, dict):
                raise ValueError(
                    "ImpulseDicts are initialized with a `dict` of top-level impulse responses."
                )
    ...
    def binary_operation(self, other, op):
        if isinstance(other, (SteadyStateDict, ImpulseDict)):
            ...
            for b in self.internals:
                ...
                internals[b] = {
                    k: op(v, other_internals[k]) for k, v in self.internals[b].items()
                }
            ...
            return NotImplementedError(
                f"Can only perform operations with ImpulseDicts and other ImpulseDicts, SteadyStateDicts, or numbers, not {type(other).__name__}"
            )
    ...
    def infer_length(self):
        ...
        if length != min(lengths):
            raise ValueError(
                f"Building ImpulseDict with inconsistent lengths {max(lengths)} and {min(lengths)}"
            )
        return length
    ...
    def get(self, k):
        ...
        if isinstance(k, str):
            ...
            try:
                return type(self)(
                    {ki: self.toplevel.get(ki, np.zeros(self.T)) for ki in k}, T=self.T
                )
            except TypeError:
                raise TypeError(
