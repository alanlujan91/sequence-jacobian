...
class NestedDict:
    def __init__(
        self,
        nesteddict,
        outputs: OrderedSet = None,
        inputs: OrderedSet = None,
        name: str = None,
    ):
    ...
    def __getitem__(self, x):
        if isinstance(x, str):
            ...
            if isinstance(o, str):
                ...
                return type(self)(
                    {oo: subdict(self.nesteddict[oo], i) for oo in o}, o, i
                )
...
class JacobianDict(NestedDict):
    def __init__(
        self, nesteddict, outputs=None, inputs=None, name=None, T=None, check=False
    ):
        if check:
    ...
    def remap(self, x: Bijection):
        ...
        return JacobianDict(
            nesteddict, inputs=x @ self.inputs, outputs=x @ self.outputs
        )
    ...
    def compose(self, J):
        ...
        if self.T is not None and J.T is not None and self.T != J.T:
            raise ValueError(
                f"Trying to multiply JacobianDicts with inconsistent dimensions {self.T} and {J.T}"
            )
    ...
    def pack(self, T=None):
        if T is None:
            if self.T is not None:
                ...
                raise ValueError(
                    "Trying to pack {self} into matrix, but do not know {T}"
                )
        else:
            if self.T is not None and T != self.T:
                raise ValueError(
                    "{self} has dimension {self.T}, but trying to pack it with alternate dimension {T}"
                )
...
class FactoredJacobianDict:
    def __init__(self, jacobian_dict: JacobianDict, T=None):
        if jacobian_dict.T is None:
            if T is None:
                raise ValueError(
                    f"Trying to factor (solve) {jacobian_dict} but do not know T"
                )
            self.T = T
        else:
        ...
        if len(self.targets) != len(self.unknowns):
            raise ValueError(
                "Trying to factor JacobianDict unequal number of inputs (unknowns)"
    ...
    def __repr__(self):
        return (
            f"<{type(self).__name__} unknowns={self.unknowns}, targets={self.targets}>"
        )

    # TODO: test this
    def to_jacobian_dict(self):
        return JacobianDict.unpack(
            -factored_solve(self.H_U_factored, np.eye(self.T * len(self.unknowns))),
            self.unknowns,
            self.targets,
            self.T,
        )
...
def ensure_valid_jacobiandict(d):
    ...
    if d and not isinstance(d, JacobianDict):
        ...
        if not isinstance(next(iter(d.keys())), str):
            raise ValueError(
                f"The dict argument {d} must have keys with type `str` to indicate `output` names."
            )
        ...
        if isinstance(jac_o_dict, dict):
            if jac_o_dict:
                if not isinstance(next(iter(jac_o_dict.keys())), str):
                    raise ValueError(
                        f"The values of the dict argument {d} must be dicts with keys of type `str` to indicate"
                        f" `input` names."
                    )
                jac_o_i = next(iter(jac_o_dict.values()))
                if not isinstance(jac_o_i, Jacobian):
                    raise ValueError(
                        f"The dict argument {d}'s values must be dicts with values of type `Jacobian`."
                    )
                else:
                    if (
                        isinstance(jac_o_i, np.ndarray)
                        and np.shape(jac_o_i)[0] != np.shape(jac_o_i)[1]
                    ):
                        raise ValueError(
                            f"The Jacobians in {d} must be square matrices of type `Jacobian`."
                        )
        else:
            raise ValueError(
                f"The argument {d} must be of type `dict`, with keys of type `str` and"
