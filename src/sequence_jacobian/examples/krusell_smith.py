...
def dag():
    ...
    ks_model_ss = create_model(
        [household, firm_ss, mkt_clearing], name="Krusell-Smith SS"
    )
    ...
    ss = ks_model_ss.solve_steady_state(
        calibration, unknowns_ss, targets_ss, solver="brentq"
    )
...
def remapped_dag():
    ...
    hh_patient = household.remap({k: k + "_patient" for k in to_map}).rename(
        "hh_patient"
    )
    hh_impatient = household.remap({k: k + "_impatient" for k in to_map}).rename(
        "hh_impatient"
    )
    ...
    calibration = {
        ...
        "nA": 100,
        "amax": 1000,
        "beta_impatient": 0.985,
        "mass_patient": 0.5,
    }
    unknowns_ss = {"beta_patient": (0.98 / 1.01, 0.999 / 1.01)}
    targets_ss = {"asset_mkt": 0.0}
    ss = ks_remapped_ss.solve_steady_state(
        calibration, unknowns_ss, targets_ss, solver="brentq"
    )
